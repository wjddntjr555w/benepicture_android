...
package com.bene.pictures.ui.widget.recyclerview;
 ...
 */
public class ObservableHorizontalRecyclerView extends RecyclerView implements Scrollable {

    private static int recyclerViewLibraryVersion = 22;

    // Fields that should be saved onSaveInstanceState
    private int mPrevFirstVisiblePosition;
    private int mPrevFirstVisibleChildWidth = -1;
    private int mPrevScrolledChildrenWidth;
    private int mPrevScrollX;
    private int mScrollX;
    private SparseIntArray mChildrenWidths;
    ...
    public ObservableHorizontalRecyclerView(Context context) {
        super(context);
        init();
    }

    public ObservableHorizontalRecyclerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public ObservableHorizontalRecyclerView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        mPrevFirstVisibleChildWidth = ss.prevFirstVisibleChildWidth;
        mPrevScrolledChildrenWidth = ss.prevScrolledChildrenWidth;
        mPrevScrollX = ss.prevScrollX;
        mScrollX = ss.scrollX;
        mChildrenWidths = ss.childrenWidths;
    ...
    public Parcelable onSaveInstanceState() {
        ...
        ss.prevFirstVisibleChildWidth = mPrevFirstVisibleChildWidth;
        ss.prevScrolledChildrenWidth = mPrevScrolledChildrenWidth;
        ss.prevScrollX = mPrevScrollX;
        ss.scrollX = mScrollX;
        ss.childrenWidths = mChildrenWidths;
        return ss;
    }
    ...
    protected void onScrollChanged(int l, int t, int oldl, int oldt) {
        ...
        if (getChildCount() > 0) {
            ...
            for (int i = firstVisiblePosition, j = 0; i <= lastVisiblePosition; i++, j++) {
                ...
                if (child != null) {
                    if (mChildrenWidths.indexOfKey(i) < 0 || (child.getWidth() != mChildrenWidths.get(i))) {
                        childWidth = child.getWidth();
                    }
                }
                mChildrenWidths.put(i, childWidth);
            }
            ...
            if (firstVisibleChild != null) {
                if (mPrevFirstVisiblePosition < firstVisiblePosition) {
                    // scroll down
                    int skippedChildrenWidth = 0;
                    if (firstVisiblePosition - mPrevFirstVisiblePosition != 1) {
                        for (int i = firstVisiblePosition - 1; i > mPrevFirstVisiblePosition; i--) {
                            if (0 < mChildrenWidths.indexOfKey(i)) {
                                skippedChildrenWidth += mChildrenWidths.get(i);
                            } else {
                                // Approximate each item's width to the first visible child.
                                // It may be incorrect, but without this, scrollX will be broken
                                // when scrolling from the bottom.
                                skippedChildrenWidth += firstVisibleChild.getWidth();
                            }
                        }
                    }
                    mPrevScrolledChildrenWidth += mPrevFirstVisibleChildWidth + skippedChildrenWidth;
                    mPrevFirstVisibleChildWidth = firstVisibleChild.getWidth();
                    ...
                    mPrevScrolledChildrenWidth -= firstVisibleChild.getWidth() + skippedChildrenWidth;
                    mPrevFirstVisibleChildWidth = firstVisibleChild.getWidth();
                } else if (firstVisiblePosition == 0) {
                    mPrevFirstVisibleChildWidth = firstVisibleChild.getWidth();
                    mPrevScrolledChildrenWidth = 0;
                }
                if (mPrevFirstVisibleChildWidth < 0) {
                    mPrevFirstVisibleChildWidth = 0;
                }
                mScrollX = mPrevScrolledChildrenWidth - firstVisibleChild.getLeft() + getPaddingLeft();
                mPrevFirstVisiblePosition = firstVisiblePosition;

                dispatchOnScrollChanged(mScrollX, mFirstScroll, mDragging);
                if (mFirstScroll) {
                    mFirstScroll = false;
                }

                if (mPrevScrollX < mScrollX) {
                    //down
                    mScrollState = ScrollState.LEFT;
                } else if (mScrollX < mPrevScrollX) {
                    //up
                    mScrollState = ScrollState.RIGHT;
                } else {
                    mScrollState = ScrollState.STOP;
                }
                mPrevScrollX = mScrollX;
            }
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getActionMasked()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                float diffX = ev.getX() - mPrevMoveEvent.getX();
                mPrevMoveEvent = MotionEvent.obtainNoHistory(ev);
                if (getCurrentScrollX() - diffX <= 0) {
    ...
    public void scrollHorizontallyTo(int x) {
        View firstVisibleChild = getChildAt(0);
        if (firstVisibleChild != null) {
            int baseWidth = firstVisibleChild.getWidth();
            int position = x / baseWidth;
            scrollHorizontallyToPosition(position);
        }
    }
     ...
     */
    public void scrollHorizontallyToPosition(int position) {
    ...
    private void init() {
        mChildrenWidths = new SparseIntArray();
        checkLibraryVersion();
    }
    ...
    private void dispatchOnScrollChanged(int scrollX, boolean firstScroll, boolean dragging) {
        if (mCallbacks != null) {
            mCallbacks.onScrollChanged(scrollX, firstScroll, dragging);
        }
        if (mCallbackCollection != null) {
            for (int i = 0; i < mCallbackCollection.size(); i++) {
                ObservableScrollViewCallbacks callbacks = mCallbackCollection.get(i);
                callbacks.onScrollChanged(scrollX, firstScroll, dragging);
            }
        }
    }
    ...
    static class SavedState implements Parcelable {
        ...
        int prevFirstVisibleChildWidth = -1;
        int prevScrolledChildrenWidth;
        int prevScrollX;
        int scrollX;
        SparseIntArray childrenWidths;
        ...
        private SavedState(Parcel in) {
            ...
            prevFirstVisibleChildWidth = in.readInt();
            prevScrolledChildrenWidth = in.readInt();
            prevScrollX = in.readInt();
            scrollX = in.readInt();
            childrenWidths = new SparseIntArray();
            final int numOfChildren = in.readInt();
            if (0 < numOfChildren) {
                for (int i = 0; i < numOfChildren; i++) {
                    final int key = in.readInt();
                    final int value = in.readInt();
                    childrenWidths.put(key, value);
                }
            }
        }
        ...
        public void writeToParcel(Parcel out, int flags) {
            ...
            out.writeInt(prevFirstVisibleChildWidth);
            out.writeInt(prevScrolledChildrenWidth);
            out.writeInt(prevScrollX);
            out.writeInt(scrollX);
            final int numOfChildren = childrenWidths == null ? 0 : childrenWidths.size();
            out.writeInt(numOfChildren);
            if (0 < numOfChildren) {
                for (int i = 0; i < numOfChildren; i++) {
                    out.writeInt(childrenWidths.keyAt(i));
                    out.writeInt(childrenWidths.valueAt(i));
                }
            }
        }
